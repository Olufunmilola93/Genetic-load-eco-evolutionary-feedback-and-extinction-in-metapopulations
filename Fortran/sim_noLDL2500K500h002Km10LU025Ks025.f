      ...
      integer(KIND=8) k5,tsteps,mig,nnew,c3,j3,j6,c1,mig2
      real(KIND=8) sel,dom,migrate,rmax,alpha,x,y,nav,t1,t2
      real(KIND=8) temp,temp1,temp2,theta,temp3,loglimit,mut
      real(KIND=8) temp4,temp5,temp6,t2a,t3a,t4a,t1a,x2
      integer(KIND=8),allocatable, dimension(:)::pop_deme
      integer(KIND=8),allocatable, dimension(:,:)::tseriesN
      real(KIND=8), allocatable, dimension(:,:)::pfreq
      real(KIND=8), allocatable, dimension(:):: freq_hab1,pop_hab1
      real(KIND=8), allocatable, dimension(:):: gdiv_hab1,gdiv_hab2
      real(KIND=8), allocatable, dimension(:):: fitness,npav
      real(KIND=8), allocatable, dimension(:)::freq_hab2,pop_hab2
      integer(KIND=8) ZBQLBIN, ZBQLGEO, ZBQLPOI


      ndemes=100
      
      rmax=0.1d+0 ! baseline growth rate
      K=500       
      L=2500
      sel=0.25d+0/K  
      dom=0.02d+0
      ...
      allocate(pfreq(L,ndemes),pop_deme(ndemes),npav(L))
      allocate(freq_hab1(tsteps/20),freq_hab2(tsteps/20))
      allocate(gdiv_hab1(tsteps/20),gdiv_hab2(tsteps/20))
      allocate(pop_hab1(tsteps/20),pop_hab2(tsteps/20))
      allocate(tseriesN(10,tsteps/20),fitness(ndemes))
...
!      create initial condition
       ...
       enddo
        ...
        do k5=1,tsteps
        if (mod(k5,100)==0)then
        print*, k5
        endif
 !      mutation
         do i=1,ndemes
         if (pop_deme(i)>0)then
         do j=1,L
         pfreq(j,i)=pfreq(j,i)+mut*(1.0d+0-2.0d+0*pfreq(j,i))
         enddo
         endif
         enddo
 
 !      deterministic effect of migration on allele frequencies and population size
         ...
         nav=nav+pop_deme(i)
         do j=1,L
         npav(j)=npav(j)+pop_deme(i)*pfreq(j,i)
         enddo         
         enddo
         nav=nav/ndemes
         do j=1,L
         npav(j)=npav(j)/ndemes
         enddo         
  ...
  1      call random_number(y)
         ...
         mig=min(mig,pop_deme(i))
         
         mig2=0
         temp1=1.0D+0
         temp3=exp(-migrate*nav)
  3      call random_number(y)
         temp1=temp1*y
         if  (temp1>temp3)then
         mig2=mig2+1
         go to 3
         ...
         if (pop_deme(i)-mig+mig2>0)then
         do j=1,L
         pfreq(j,i)=(pop_deme(i)-mig)*pfreq(j,i)+mig2*(npav(j)/nav)
         enddo
         pop_deme(i)=pop_deme(i)-mig+mig2
         do j=1,L
         pfreq(j,i)=pfreq(j,i)/pop_deme(i)
         enddo
         endif
         enddo
...
!       selection
        ...
        ! calculate mean fitness of each deme, new allele freq at each locus in each deme + new population size
        
         do i=1,ndemes
         fitness(i)=1.0d+0
         do j=1,L
         temp=1.0d+0-2.0d+0*pfreq(j,i)*(1.0d+0-exp(-dom*sel))+
     c   pfreq(j,i)*pfreq(j,i)*(1.0d+0-2.0d+0*exp(-dom*sel)+exp(-sel))
         fitness(i)=fitness(i)*temp
         pfreq(j,i)=(pfreq(j,i)/temp)*
     c   (pfreq(j,i)*exp(-sel)+(1.0d+0-pfreq(j,i))*exp(-dom*sel))
         enddo
         ...
         ! stochastically generate new population sizes and allele frequencies
         do i=1,ndemes
         ! new pop size
         temp=exp(rmax-(rmax/K)*pop_deme(i))*fitness(i)*pop_deme(i)
 
         pop_deme(i)=ZBQLPOI(temp)

!         new allele frequency
          if (pop_deme(i)>0)then
          do j=1,L
          if (pfreq(j,i)<0.0d+0 .or. pfreq(j,i)>1.0d+0)then
          print*, i,j,pfreq(j,i)
          endif  
          j3=ZBQLBIN(2*pop_deme(i),pfreq(j,i))
          pfreq(j,i)=(j3*0.5d+0)/pop_deme(i)
          enddo
          endif
 ...
 ! calculate statistics
         ...
         temp4=0.0D+0
         do i=1,ndemes
         temp4=temp4+theta*pop_deme(i)
         if(mod(i,10)==0)then
         tseriesN(i/10,k5/20)=pop_deme(i)
         endif
         enddo
         pop_hab1(k5/20)=temp4/ndemes
         ...
         if (pop_deme(i)>0)then
         do j=1,L
         temp4=temp4+theta*pfreq(j,i)
         temp5=temp5+theta*pfreq(j,i)*(1.0d+0-pfreq(j,i))
         enddo
         c1=c1+1
         endif
         enddo
         freq_hab1(k5/20)=temp4/(c1*L)
         gdiv_hab1(k5/20)=temp5/(c1*L)
         endif
 ...
 19   open(41,file='L2500_K500_dom0.02_Km10_Ks025_LU025_statsB.txt')
      do i=1,tsteps/20
 ...
 9    Format(I8,1X,F15.9,1X,F15.9,1X,F15.9)
      ...
      open(41,file='L2500_K500_dom0.02_Km10_Ks025_LU025_tseriesB.txt')
      do i=1,tsteps/2   ! pop size in deme 10,20,30,..100  vs. generation.
      write(41,8) i*2,(tseriesN(k,i),k=1,10)
...
*
      INTEGER(KIND=8) SEED,SS,MM,HH,DD,FILNO,I
      INTEGER(KIND=8) INIT
      DOUBLE PRECISION ZBQLIX(43),B,C
...
*
      ...
      INTEGER(KIND=8) CURPOS,ID22,ID43
...
*
      ...
      INTEGER(KIND=8) N,ZBQLBIN,ZBQLGEO,IZ,NN
...
*
      ...
      INTEGER(KIND=8) ZBQLPOI,ZBQLBIN,K,INIT
      SAVE INIT,PI
      DATA INIT /0/
